...
CRITICAL: Return ONLY valid JSON. No text before or after. No markdown. No explanations. No trailing commas.
{
  ...
  // Try 1: Ollama (local, no rate limits) — with JSON mode + repair
  const ollamaUrl = process.env.OLLAMA_URL || 'http://localhost:11434';
  const ollamaModel = process.env.OLLAMA_MODEL || 'llama3:8b';
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      if (attempt > 0) {
        console.log(`[Feedback] Ollama retry (attempt ${attempt + 1})...`);
        await new Promise(r => setTimeout(r, 2000));
      }
      console.log(`[Feedback] Trying Ollama (model=${ollamaModel}, attempt ${attempt + 1})`);
      const ollamaRes = await Promise.race([
        fetch(`${ollamaUrl}/api/generate`, {
          ...
          body: JSON.stringify({
            ...
            options: { temperature: 0, num_predict: 4096 }  // Increased for longer feedback
          })
        }),
        new Promise((_, rej) => setTimeout(() => rej(new Error('TIMEOUT')), 120000))  // 120s for comprehensive feedback
      ]);
      if (!ollamaRes.ok) {
        const errText = await ollamaRes.text().catch(() => '');
        console.log(`[Feedback] Ollama HTTP ${ollamaRes.status}: ${errText.substring(0, 100)}`);
        break;  // HTTP error = don't retry
      }
      const data = await ollamaRes.json();
      const text = data.response || '';
      console.log(`[Feedback] Ollama raw length: ${text.length} chars`);

      // Try direct parse first, then repair
      const parsed = repairJSON(text);
      if (parsed && parsed.overallSummary) {
        console.log('[Feedback] ✓ Generated via Ollama');
        return parsed;
      }
      console.log(`[Feedback] Ollama JSON incomplete (attempt ${attempt + 1})`);
    } catch (e) {
      console.log(`[Feedback] Ollama attempt ${attempt + 1}: ${e.message?.substring(0, 60)}`);
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        console.log('[Feedback] ✓ Generated via Gemini');
        const parsed = repairJSON(text);
  ...
  console.log('[Feedback] Both Ollama and Gemini unavailable');
  return null;
}
...
router.post('/:studentId', async (req, res) => {
  try {
    ...
    // Generate AI feedback (Ollama → Gemini → null)
