...
// ──────────────── Configuration ────────────────
// Priority: Ollama → Gemini → Smart Local
...
// ──── 1. Ollama API (local, no rate limits) ────
async function callOllama(prompt) {
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      if (attempt > 0) {
        console.log(`[Ollama] Retry attempt ${attempt}...`);
        await new Promise(r => setTimeout(r, 1000));
      }
      console.log(`[Ollama] Calling model=${OLLAMA_MODEL} (attempt ${attempt + 1})`);
      ...
      if (!res.ok) {
        ...
        console.log(`[Ollama] HTTP ${res.status}: ${err.substring(0, 100)}`);
        return null;  // HTTP error = don't retry, model probably not found
      }
      ...
      if (err.message === 'TIMEOUT') {
        console.log(`[Ollama] Timeout (30s)`);
        return null;  // Timeout = don't retry
      }
      // JSON parse error from parseEvalJSON → retry once
      console.log(`[Ollama] Attempt ${attempt + 1} error: ${err.message?.substring(0, 80)}`);
...
// ──── 2. Gemini API (cloud, rate limited) ────
...
// ──── Main evaluator: Ollama → Gemini → Local ────
async function evaluateTranscript(transcript, questionPrompt, evaluationCriteria) {
  ...
  // Try 1: Ollama (local — no rate limits, no cost)
  console.log(`[Eval] Trying Ollama (model=${OLLAMA_MODEL}, url=${OLLAMA_URL})...`);
  const ollamaResult = await callOllama(prompt);
  if (ollamaResult) {
    console.log(`[Eval] ✓ Ollama score=${ollamaResult.score}`);
    return { ...ollamaResult, source: 'ollama' };
  }

  // Try 2: Gemini API (cloud — may be rate limited)
  console.log(`[Eval] Ollama unavailable, trying Gemini...`);
  ...
  // Try 3: Smart local NLP evaluator (always works)
  console.log(`[Eval] Both APIs unavailable, using local NLP evaluator`);
